...
describe.runIf(nativeAvailable)("authenticated directory import", () => {
  ...
  it("imports direct schema-2 assets and an exact opaque companion with truthful provenance 3", async () => {
    ...
    expect(provenance.records[0]).toMatchObject({ type: "asset", assetId: "direct", source: { kind: "directory", collectionId: "icons", sourcePath: "icons/direct.svg", sourceState: "present", canonicalState: "present", resolution: "aligned", toolVersion: TOOL_VERSION }, migration: null, normalizationPolicy: null });
  ...
  it("normalizes exact-common input and records only the consumed policy on the asset", async () => {
    ...
    const provenance = unwrap(parseImportProvenanceV3(await readFile(join(root, ".tfsb", "provenance.json"), "utf8")));
    expect(provenance.records[0]).toMatchObject({ normalizationPolicy: { policyBasis: "tfsb-normalization-policy-v1", implementationVersion: TOOL_VERSION }, source: { sourceCanonicalDigest: expect.stringMatching(/^sha256:/), canonicalDigest: expect.stringMatching(/^sha256:/) } });
  });
